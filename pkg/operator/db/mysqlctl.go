...
func (op *OperatorMysql) Open(ctx context.Context, dbName operator.DBType, config operator.DBConnInfo) (operator.DBOperator, error) {
	protocol := "tcp"
...
func (op *OperatorMysql) GetType() operator.DBType {
	return operator.MySQL
}

func (op *OperatorMysql) Ping(ctx context.Context) error {
	return op.db.PingContext(ctx)
}

func (op *OperatorMysql) Close(ctx context.Context) error {
	return op.db.Close()
}
